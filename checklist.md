...
## 주 보기 날짜 줄을 오늘 중심으로 (2026-08-08)

요청: "상단의 오늘 날짜가 가운데에 나오도록". 월~일 고정 창을 버리고 **기준일 중심 7일**(앞 3 + 기준일 + 뒤 3)로 바꿨다. 요일 순서가 기준일에 따라 회전한다는 맞바꿈을 사용자에게 알리고 진행.

- [x] `calendar.js` — `weekGrid()` 를 기준일 중심 7일로 (이름·시그니처 유지 → 호출부 수정 0)
- [x] `calendar.js` — `calendarWeekEnds()` 추가 (월 보기 Home/End 가 쓰던 월~일 양끝)
- [x] `main.js` — Home/End 를 보기별로 분기
- [x] 테스트 3건 추가 — 가운데 배치 / 월 경계 / 월 보기 Home·End
- [x] `npm test` **49 통과**(12 + 26 + 11)

## 검증 (2026-08-08)
- [x] 앱을 열면 오늘(8/8)이 4번째 칸 — `수 5 · 목 6 · 금 7 · **토 8** · 일 9 · 월 10 · 화 11`
- [x] 날짜를 골라도 창이 안 움직인다(오늘은 계속 가운데). 창은 ‹ › 와 `오늘` 로만 움직인다
- [x] `›` → 8/12~18 로 7일 이동, 고른 날도 같은 요일(토 15)로 따라감
- [x] `오늘` → 다시 가운데 복귀
- [x] 390px 한 칸 50.1px 유지, 일요일 붉은색·오늘 잉크 블록 그대로

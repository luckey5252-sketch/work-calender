...
## 모바일 주 보기 = 3일 (2026-08-07)

- **문제를 추측하지 않고 실제로 봤다.** 사용자의 "화면짜임새가 이상하다"는 서술만으로는 무엇이 깨졌는지 알 수 없어, 라이브를 390px 폭으로 렌더해 확인했다. 네 가지가 나왔고 그중 셋만 이번 범위다.
- **390px 렌더 방법** — 브라우저 창 리사이즈가 먹지 않아(창이 최대화 상태, `innerWidth` 가 1536 그대로) **같은 출처 iframe 을 390px 로 띄웠다.** 미디어 쿼리는 iframe 뷰포트 기준으로 평가되므로 폰 레이아웃이 그대로 재현된다. 창 크기에 의존하지 않아 다음에도 이 방법을 쓸 것.
- **3일 폭 계산에 gap 을 반드시 빼야 한다.** `grid-auto-columns: 33.33%` 로 하면 gap 12px 때문에 세 번째 칸이 잘린다. `--week-gap` 을 변수로 빼고 `calc((100% - 2 * var(--week-gap)) / 3)` 로 잡았다. 실측 116×3 + 6×2 = 360 ≈ 359(clientWidth).
- **`white-space: nowrap` 이 숨어 있던 결함을 드러냈다.** 버튼 글씨 접힘을 막으려고 nowrap 을 넣자 `.view-toggle` 이 flex 로 눌리며 **'주' 버튼이 통째로 사라졌다** — `.view-toggle` 에 `overflow: hidden` 이 있어 넘친 부분이 잘린 것이다. 원래도 눌리고 있었는데 글씨가 접혀서 안 보였을 뿐이다. `flex-shrink: 0` 으로 막았다. **overflow:hidden 컨테이너를 flex 자식으로 둘 땐 shrink 를 반드시 막을 것.**
- **자동 가로 이동은 `selected` 기준이고, 다른 주로 넘기면 일부러 아무것도 안 한다.** `‹ ›` 는 `cursor` 만 바꾸고 `selected` 는 오늘에 남는다. 그 주에 선택일이 없으면 `scrollWeekToSelected()` 가 그냥 빠져 스크롤이 0(월요일)에서 시작한다 — 다른 주를 볼 땐 그 주의 처음부터 보는 게 맞다.
- **스크롤 위치는 render 마다 새로 계산된다.** `renderCalendar` 가 `root.innerHTML=''` 로 다시 그리므로 `scrollLeft` 는 매번 0으로 리셋된다. 상태로 들고 있을 필요가 없어 `render()` 끝에서 한 번 호출하는 것으로 충분하다.
- **CSS 변경이라 단위 테스트가 없다.** `render.js` 는 안 건드렸고 `main.js` 는 테스트 대상이 아니다. 대신 iframe 에서 `offsetLeft`·`clientWidth`·`scrollLeft` 를 실측해 숫자로 확인했다. 기존 64건은 회귀 확인용으로 돌렸다(전부 통과).
- **남은 것 — 월 보기 칩.** 390px에서 월 보기는 칸이 55px라 칩에 `신규`·`수정` 배지만 보이고 제목이 안 보인다. 무슨 일정인지 알 수 없어 사실상 쓸모가 없다. 이번 요청(주 보기 3일) 범위 밖이라 손대지 않았다.
- **시간 표시는 여전히 모바일에서 숨겨져 있다**(`.chip-time { display: none }`, ≤760px). 칸이 1/3로 더 좁아져 유지했으나, 업무 달력에서 시간이 안 보이는 건 손실이다. 분류 라벨(회의/출장/기타)은 이제 색을 인코딩하지 않으므로(칩 색은 `headAttending` 만 결정) 접근성 부담이 없다 — 좁은 칸에서 **분류 라벨 대신 시간을 보이는 맞바꿈**이 가능하다. 사용자 판단 필요.

## 모바일 월 보기 = 제목 + 시작시간 (2026-08-07)

- **사용자가 방향을 직접 정했다.** 내가 세 가지 안(점+목록 / 점+주보기이동 / 날짜만)을 물었으나 사용자는 그걸 고르지 않고 **"제목과 시작시간만 표시"** 라고 답했다. 즉 월 보기에서도 무슨 일정인지 글자로 읽고 싶다는 것이고, 점·마커 방향은 기각됐다.
- **끝시간을 빼는 건 CSS로 못 한다.** `timeLabel()` 이 "09:30–11:00" 을 한 문자열로 만들어 넣으므로 시작시간만 남기려면 렌더가 달라져야 한다. `startTimeLabel()` 을 따로 두고 `chip()` 에 `showStartTime` 옵션을 더해 **월 보기에서만** 별도 `span.chip-start` 를 붙였다. 넓은 화면에선 CSS가 이걸 숨기고 기존 `.chip-time`(범위)을 그대로 쓴다 — **뷰포트 판단은 CSS에, 데이터 선택은 렌더에** 둔 분업이다.
- **`grid-auto-rows: minmax(_, 1fr)` 이 빈 주를 부풀린다.** 1fr 은 행끼리 높이를 맞추므로 일정 3건짜리 주 하나가 **여섯 주 전부**를 179px로 만들었다(전체 1095px). `auto` 로 바꾸니 614px. 좁은 화면에서 행 높이를 균등하게 맞출 이유가 없다.
- **48px에서 한글은 한 줄 4자다.** 여백을 최대한 짜내(cal-root 14→8, gap 4→3, day 4→2, chip 4→3) 제목 폭 35px 확보 → 두 줄 8자. 글자 크기를 더 줄이는 건 업무용으로 역효과라 하지 않았다. **이게 물리적 한계고, 더 보이게 하려면 7칸을 포기하는 수밖에 없다.**
- **CLAUDE.md 와의 맞바꿈을 의식적으로 했다.** "변경 표시는 색만 쓰지 말고 라벨 병행"인데 월 보기 좁은 칸에서는 `신규`/`수정` 배지를 뺐다(선만 남음). 사용자 지시("제목과 시작시간만")가 명시적이었고, 배지는 주 보기·상세에 그대로 있다. 본부장은 굵은 제목이 비-색 단서를 계속 진다.
- **브라우저 모듈 캐시에 두 번 속을 뻔했다.** CSS는 새것이 먹었는데 `render.js` 는 캐시된 옛 모듈이 돌아 `chip-start` 가 DOM에 없었다. `fetch('/src/render.js')` 로 확인했더니 그것도 캐시라 `false` 가 나왔고, `{cache:'no-store'}` 로 다시 받아서야 서버 파일이 새것임을 확인했다. **iframe 미리보기로 CSS를 고칠 땐 JS 변경이 섞이면 ctrl+shift+r 로 하드 리로드할 것.**
- **스크린샷을 축소해서 읽다 색을 오독했다.** 세그먼트 `월/주` 가 잘못 칠해진 줄 알았으나 `getComputedStyle` 로 재보니 선택된 쪽이 정상적으로 진한 배경이었다. **작은 UI 색 판단은 스크린샷이 아니라 계산된 스타일로 확인할 것.**

## 주 보기 = 날짜 줄 + 하루 상세 (2026-08-08)

- **사용자가 두 갈래를 직접 골랐다.** 아이폰 캘린더 스크린샷을 주며 "위에 일자, 아래 세부일정" 을 요청 → 내가 (1) 하단을 **시간순 목록** vs **시간축 타임라인**, (2) **폰만** vs **폰·데스크톱 공통** 으로 나눠 물었고 **목록 + 공통** 을 골랐다.
- **타임라인을 안 쓴 근거는 데이터 모델이다.** `end` 가 선택 항목이라(`render.js` 의 `if (ev.time.end)`) 끝시간 없는 일정은 블록 높이를 정할 수 없다. 임의로 1시간이라 가정하면 없는 정보를 지어내는 셈이다. 종일 일정도 시간축에 놓을 자리가 없다. 스크린샷 자체가 반례이기도 했다 — 화면 전체가 새벽 12~10시고 실제 일정은 하나였다.
- **날짜 칸의 의미가 보기마다 달라졌다.** 월 보기 칸 클릭은 여전히 폼을 열지만, 주 보기 날짜 줄은 **고르기**만 한다(아래 상세를 바꿀 뿐). 렌더가 `data-pick` 을 붙이고 `onRootClick` 이 그걸 먼저 잡는다. 일정 추가 진입점은 상단 `#add` 와 빈 날의 `일정 더하기` 가 맡는다.
- **주를 넘기면 고른 날도 옮겨야 한다.** 안 그러면 상단 줄에 없는 날의 일정이 아래 남는다. `stepPeriod()` 가 **이전 주에서의 요일 index** 를 찾아 새 주의 같 은 자리로 옮긴다(못 찾으면 그 주 월요일). 월 보기에서 다른 주의 날을 고른 채 주 보기로 넘어올 수도 있어 `#view-week` 는 `cursor = selected` 로 맞춘다.
- **클래스 이름을 바꾸면서 공유 선택자 목록을 놓쳐 색이 죽었다.** `.week-dow`→`.strip-dow`, `.week-daynum`→`.strip-daynum` 으로 바꿨는데 토·일 색(`.week-dow.is-sun`)과 공휴일 색(`.week-daynum.is-holiday`) 규칙이 **월 보기와 공유되는 다른 줄에** 있어 함께 안 고쳐졌다. 화면상 티가 안 나 `getComputedStyle` 로 재보고서야 잡았다 — 토·일이 `ink-soft`, 공휴일 숫자가 `ink` 였다. **클래스를 rename 할 땐 그 이름을 grep 해서 흩어진 선택자 목록까지 훑을 것.**
- **넓은 화면에선 칩을 접으면 안 된다.** 처음엔 폰과 같이 `flex-wrap: wrap` 으로 뒀는데 1536px 에서 칩 폭이 1470px 라 글자가 왼쪽 1/5 에 몰리고 나머지가 빈 띠였다(본부장 칩은 파란 통 띠). 넓은 화면은 **한 줄**(제목에 `margin-right:auto` → 장소·부서가 오른쪽 끝)로, 좁은 화면만 미디어쿼리에서 wrap. 칩 높이 33px vs 69px.
- **≤760px 의 `.chip-time { display:none }` 을 하루 상세에서만 되살렸다.** 좁은 월 보기에서 제목을 살리려 시간을 감춘 규칙인데, 하루 상세는 "몇 시 일정인가" 가 본론이라 예외를 뒀다(`.day-panel .chip-time` 이 특정성으로 이김).
- **장소를 칩에 새로 넣었다**(`showLocation`). 요청은 배치였지만 "세부일정" 이라 했고 하단 칸이 화면 폭 전체라 자리가 난다. `장소 · 담당부서` 한 줄 — 구분자는 `.chip-loc + .chip-dept::before` 라 장소가 없으면 점도 안 나온다.
- **주 보기에서 전체 `empty-hint` 를 껐다.** 하루 패널이 이미 빈 상태와 `일정 더하기` 를 주므로 겹친다. 월 보기에서만 남긴다.
- **날짜 줄을 sticky 로 뒀다.** 일정이 많은 날에 아래로 스크롤해도 다른 날로 건너갈 수 있어야 해서다. 조상에 `overflow` 가 없어 뷰포트가 스크롤 컨테이너라 그대로 먹는다.
- **테스트의 시간대 의존을 고정했다.** 스모크의 일정들은 UTC 문자열이라 "어느 날에 속하는가" 가 로컬 시대에 달렸는데, 새 주 보기 테스트는 "6/25 하루에 셋 다" 를 전제한다. `process.env.TZ='Asia/Seoul'` 를 **본문 첫 줄**에 뒀다 — `import` 는 끌어올려지므로 그 위에 쓰는 건 의미가 없고, 이 위치가 이 파일에서 Date 를 만들기 전 가장 이른 지점이다.
- **`scrollWeekToSelected()` 는 지웠다.** 가로 스크롤 3일 배치가 사라져 붙잡을 대상이 없다. 주 보기 ↑↓ 도 ±1일에서 ±7일로 바꿨다 — 날짜 줄이 가로 한 줄이라 상하가 주 이동인 게 맞다.

## 날짜 줄을 오늘 중심 7일로 (2026-08-08)

- **월~일 고정을 포기한 것이 이 변경의 본질이다.** 7칸 중 가운데에 오늘을 두려면 창이 달력 주가 아니라 기준일 중심으로 굴러가야 한다. 요일 순서가 회전하고(수·목·금·토·일·월·화) "이번 주" 감각이 사라지는데, 사용자에게 먼저 알리고 진행했다. CLAUDE.md 의 "주 시작 월요일" 은 이제 **월 보기에만** 적용된다.
- **`weekGrid` 의 이름과 시그니처를 유지해 호출부를 하나도 안 고쳤다.** 몸통만 `startOfWeek/endOfWeek` → `addDays(date,-3)` 기준으로 바꿨다. render.js 의 스트립, main.js 의 `weekRangeLabel`·`stepPeriod`·키보드 경계 검사가 전부 그대로 동작한다. 이름이 여전히 "이 보기가 그리는 7일" 을 뜻하므로 거짓말도 아니다.
- **그런데 `weekGrid` 를 월 보기도 쓰고 있었다 — 키보드 Home/End.** 몸통을 바꾸니 월 보기에서 "주의 처음/끝" 이 "3일 전/후" 로 조용히 바뀌었다. 테스트가 못 잡았고(월 보기 키보드 테스트가 없다) 화면에도 안 드러난다. `calendarWeekEnds()` 를 따로 빼고 Home/End 를 보기별로 분기했다. **공용 함수의 의미를 바꿀 땐 이름을 grep 해서 다른 보기가 쓰고 있는지 먼저 볼 것** — 이번엔 render 만 보고 지나칠 뻔했다.
- **창은 날짜를 골라도 안 움직인다.** 스트립 클릭은 `selected` 만 바꾸고 `cursor` 는 그대로라, 오늘이 계속 가운데에 있다. 창을 움직이는 건 `‹ ›`(cursor ±7일)와 `오늘`(cursor=오늘) 뿐이다. 클릭마다 창이 재중심화되면 누를 때마다 날짜가 발밑에서 미끄러진다.
- **또 모듈 캐시에 속았다(두 번째).** `calendar.js` 를 고치고 iframe 을 새 쿼리로 다시 띄웠는데도 옛 모듈이 돌아 월~일이 그대로 나왔다. `curl` 로 서버 파일이 새것임을 먼저 확인한 뒤 `ctrl+shift+r` 로 해결. **iframe src 에 쿼리를 붙이는 건 import 된 모듈까지 무효화하지 못한다** — 노트에 이미 적어둔 함정을 그대로 다시 밟았다.
- **rename 잔재 두 개를 배포 후에야 잡았다.** 라이브 CSS 를 `grep` 해보니 `.week-col` 이 1건 남아 있었다 — `prefers-reduced-motion` 블록의 `.day, .week-col { transition: ... }` 이라 주 보기 섹션을 통째로 갈아끼울 때 시야 밖이었다(날짜 칸 전환 효과가 조용히 죽어 있었다). 더 나쁜 건 `.col-empty` 로, CSS 규칙은 지웠는데 **사용자 관리 목록의 에러 메시지**가 아직 그 클래스를 쓰고 있었다(`main.js`). 주 보기용으로 지은 이름을 다른 화면이 빌려 쓰고 있던 것 — 이제 `.users-error` 를 쓴다(에러니까 회색보다 경고색이 맞다). **클래스를 지울 땐 CSS 뿐 아니라 JS 까지 grep 할 것.**
